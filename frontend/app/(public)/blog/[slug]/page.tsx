...
async function getPost(slug: string) {
  const res = await serverApiGet<any>(`/posts/${slug}`, undefined, { revalidate: 30 })
  return res.data
}

async function getRelatedPosts(postId: string, categorySlugs: string[]) {
  // Backend doesn't expose a "related posts" endpoint.
  // Use category slug filter and exclude current post client-side.
