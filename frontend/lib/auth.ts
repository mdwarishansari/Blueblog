...
export async function getCurrentUser() {
  ...
  // Verify token and extract user info from payload
  const payload = verifyAccessToken(accessToken)
  if (!payload) return null

  // Return user info from token payload
  // Note: For full user details, you may need to call the backend API
  // but for basic auth checks, token payload is sufficient
  return {
    id: payload.userId,
    email: payload.email,
    role: payload.role,
    name: '', // Will be filled by API calls when needed
    bio: null,
    profileImage: null,
    createdAt: null,
  }
